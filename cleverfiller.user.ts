...
// @author       Joolowweng
// @license      MIT
// @downloadURL  https://github.com/joolowweng/cleverfiller/raw/refs/heads/master/dist/cleverfiller.user.js
// @updateURL    https://github.com/joolowweng/cleverfiller/raw/refs/heads/master/dist/cleverfiller.user.js
// @noframes     true
// @match        *://*/*
...
// @grant        GM_info
// @resource     index https://raw.githubusercontent.com/joolowweng/cleverfiller/refs/heads/master/html/dashboard.html
// @resource     css https://raw.githubusercontent.com/joolowweng/cleverfiller/refs/heads/master/css/style.css
// ==/UserScript==
...
const EnlistArray: Array<Record<string, any>> =
  GM_getValue(get_window_url(), default_cache).enlist || [];
// ElementCache: Array to store the fillable elements that are currently in the DOM
const PreloadArray: Array<Record<string, any>> =
  GM_getValue(get_window_url(), default_cache).preload || [];
const AfterloadArray: Array<Record<string, any>> =
  GM_getValue(get_window_url(), default_cache).afterload || [];
...
async function get_response<T>(
  options: Tampermonkey.Request<any>,
  on_start?: () => void,
): Promise<T | string> {
  ...
  return new Promise((resolve, reject) => {
    GM_xmlhttpRequest({
      ...
      onload: (response) => {
        console.log(
          "[CleverFiller] get_response(): Response Status:",
          response.status,
        );
        if (response.status === 200) {
          try {
            ...
            console.error(
              "[CleverFiller] get_response(): Failed to parse response:",
              error,
            );
          ...
          console.error(
            "[CleverFiller] get_response(): HTTP Error:",
            response.status,
          );
...
function parse_ai_response(response: any, element?: HTMLElement): string {
  try {
    ...
    if (element) {
      ...
      if (tagName === "input") {
        if (
          ...
          console.warn(
            "[CleverFiller] parse_ai_response(): Invalid email format:",
            value,
          );
          value = "";
          ...
          if (dateMatch) {
            ...
            try {
              ...
              console.warn(
                "[CleverFiller] parse_ai_response(): Invalid date format:",
                value,
              );
            }
          }
        ...
        let optionToSelect = options.find(
          (opt) => opt.text.trim().toLowerCase() === value.trim().toLowerCase(),
        );
        ...
        if (optionToSelect) {
          console.log(
            `[CleverFiller] parse_ai_response(): Select match found: "${value}" -> "${optionToSelect.text}" (${optionToSelect.value})`,
          );
          value = optionToSelect.value;
        } else {
          console.warn(
            `[CleverFiller] parse_ai_response(): No matching option found for: "${value}"`,
          );
    ...
    console.error(
      "[CleverFiller] parse_ai_response(): Failed to parse API response:",
      error,
    );
    return "";
  }
...
function create_hover_overlays(
  ...
  const existingOverlays = document.querySelectorAll(
    ".cleverfiller-hover-overlay-add, .cleverfiller-hover-overlay-remove",
  );
  ...
  for (const element of elements) {
    ...
    if (
      ...
      console.log(
        "[CleverFiller Debug] Skipping hidden or non-interactable element for overlay:",
        element,
      );
    ...
    overlay.className = isAlreadyEnlisted
      ? "cleverfiller-hover-overlay-remove"
      : "cleverfiller-hover-overlay-add";
    ...
    if (isAlreadyEnlisted) {
      ...
      overlay.addEventListener("click", (e: MouseEvent) => {
        ...
        create_hover_overlays(
          ...
          cacheArray,
          dataArray.filter(
            (d) => create_element_signature(d) !== elementSignature,
          ), // Pass remaining data
          addCallback,
          removeCallback,
          addColor,
      ...
      overlay.addEventListener("click", (e: MouseEvent) => {
        ...
        create_hover_overlays(
          ...
          [
            ...dataArray,
            extract_data_for_enlist_storage(element as HTMLElement),
          ], // Pass updated data
          addCallback,
          removeCallback,
          addColor,
...
function hover_overlay_handler_for_enlist_button(
  elements: Array<FormElement>,
): void {
  create_hover_overlays(
    elements,
...
function hover_overlay_handler_for_load_button(
  elements: Array<FormElement>,
  ...
  if (element.tagName === "INPUT" || element.tagName === "TEXTAREA") {
    ...
    if (optionToSelect) {
      ...
      setTimeout(() => {
        element.dispatchEvent(new Event("change", { bubbles: true }));
        console.log(
          `[CleverFiller] Select option set to: "${optionToSelect.value}"`,
        );
      }, 100);
    }
    ...
    console.log(
      `[CleverFiller] Button element - no value to set: "${element.textContent}"`,
    );
  }
...
const DEFAULT_LOAD_TAGS =
  'button[type="submit"], input[type="submit"], button.submit, a.btn';
...
function initialize_default_selectors(): void {
  ...
  if (GM_getValue("enlist_tags", null) === null) {
    ...
    console.log(
      "[CleverFiller] Initialized default enlist tags:",
      DEFAULT_ENLIST_TAGS,
    );
  }
  ...
  if (GM_getValue("load_tags", null) === null) {
    ...
    console.log(
      "[CleverFiller] Initialized default load tags:",
      DEFAULT_LOAD_TAGS,
    );
  }
}
...
function scan_form_elements(customSelector?: string): Array<FormElement> {
  ...
  const savedSelector =
    customSelector || GM_getValue("enlist_tags", DEFAULT_ENLIST_TAGS);
  ...
  try {
    ...
    console.log(
      `[CleverFiller] Found ${filteredInputs.length} elements with selector: ${selector}`,
    );
    ...
    const defaultInputs =
      document.querySelectorAll<FormElement>(DEFAULT_ENLIST_TAGS);
    return Array.from(defaultInputs).filter(
      (input) => !input.closest("div#cleverfiller-container"),
    );
  }
}
function get_label_text(
  ...
  if (element.id) {
    ...
    if (explicitLabel && explicitLabel.textContent) {
      console.log(
        `[CleverFiller Debug] Label found via 'for': ${explicitLabel.textContent.trim()}`,
      );
  ...
  if (labelledby) {
    ...
    if (labelElement && labelElement.textContent) {
      console.log(
        `[CleverFiller Debug] Label found via 'aria-labelledby': ${labelElement.textContent.trim()}`,
      );
  ...
  if (ariaLabel) {
    console.log(
      `[CleverFiller Debug] Label found via 'aria-label': ${ariaLabel.trim()}`,
    );
  ...
  if (parentRow) {
    ...
    const inputCellIndex = inputCell
      ? cells.indexOf(inputCell as HTMLTableCellElement)
      : -1;
    ...
    if (inputCellIndex > 0) {
      for (let i = inputCellIndex - 1; i >= 0; i--) {
        ...
        if (
          potentialLabelCell.matches(
            'td[class*="label" i], th[class*="label" i]',
          )
        ) {
          const labelElement = potentialLabelCell.querySelector("label");
          if (labelElement && labelElement.textContent) {
            console.log(
              `[CleverFiller Debug] Label found in preceding table cell (class*="label"): ${labelElement.textContent.trim()}`,
            );
            return labelElement.textContent.trim();
          }
          ...
          if (potentialLabelCell.textContent) {
            console.log(
              `[CleverFiller Debug] Label found via preceding table cell content (class*="label"): ${potentialLabelCell.textContent.trim()}`,
            );
        ...
        if (directLabel && directLabel.textContent) {
          console.log(
            `[CleverFiller Debug] Label found in preceding table cell (direct label): ${directLabel.textContent.trim()}`,
          );
    ...
    if (cells.length > 0) {
      ...
      if (firstCellLabel && firstCellLabel.textContent) {
        console.log(
          `[CleverFiller Debug] Label found via first table cell (label tag): ${firstCellLabel.textContent.trim()}`,
        );
        return firstCellLabel.textContent.trim();
      }
      // Only use first cell text if it has a label-like class
      if (
        cells[0].matches('td[class*="label" i], th[class*="label" i]') &&
        cells[0].textContent
      ) {
        console.log(
          `[CleverFiller Debug] Label found via first table cell content (class*="label"): ${cells[0].textContent.trim()}`,
        );
  ...
  while (previousSibling) {
    if (previousSibling.tagName === "LABEL") {
      if (previousSibling.textContent) {
        console.log(
          `[CleverFiller Debug] Label found via previous sibling: ${previousSibling.textContent.trim()}`,
        );
  ...
  if (container) {
    ...
    const suitableLabel = containerLabels.find(
      (lbl) =>
        !lbl.hasAttribute("for") || lbl.getAttribute("for") === element.id,
    );
    if (suitableLabel && suitableLabel.textContent) {
      console.log(
        `[CleverFiller Debug] Label found in container: ${suitableLabel.textContent.trim()}`,
      );
  ...
  if (placeholder_text) {
    console.log(
      `[CleverFiller Debug] Label found via placeholder: ${placeholder_text}`,
    );
  ...
  if (button_text) {
    console.log(
      `[CleverFiller Debug] Label found via button text: ${button_text}`,
    );
    return button_text;
  }
  ...
  // Final fallback
  console.log(
    `[CleverFiller Debug] No label found for element:`,
...
function get_element_attributes(element: HTMLElement): {
  [key: string]: string;
} {
...
function filter_redundant_attributes(
  element: HTMLElement,
...
function create_element_signature(
...
function check_if_element_exists_in_enlist_array(
  element: HTMLElement,
): boolean {
...
function extract_data_for_enlist_storage(
  element: HTMLElement,
): Record<string, any> {
  const labelText = get_label_text(element as HTMLInputElement);
  const attributeValues = filter_redundant_attributes(element);
...
function setupTabNavigation(container: HTMLDivElement): void {
  ...
  const tabContentContainer = container.querySelector<HTMLElement>(
  ...
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      const contents =
        tabContentContainer.querySelectorAll<HTMLElement>(".cf-tab-content");
      contents.forEach((content) => {
        content.classList.remove("active");
      });

      // Show the target tab content
      const targetContent = tabContentContainer.querySelector<HTMLElement>(
        `#${targetId}`,
      );
...
function setup_auto_save(container: HTMLDivElement): void {
  const api_input = container.querySelector(
    "#cf-api-input",
  ) as HTMLInputElement;
  const model_option = container.querySelector(
    "#cf-model-select",
  ) as HTMLSelectElement;
  const context_input = container.querySelector(
    "#cf-context-textarea",
  ) as HTMLTextAreaElement;
  const initial_display = container.querySelector(
    "#cf-initial-display",
  ) as HTMLInputElement;
  // 2025.04.13: Added tag inputs for saving custom selectors
  const enlist_tags = container.querySelector(
    "#cf-enlist-tags",
  ) as HTMLInputElement;
  const load_tags = container.querySelector(
    "#cf-load-tags",
  ) as HTMLInputElement;

  // 2025.04.14: Add reset buttons for tag fields
  const reset_enlist_tags_button = container.querySelector(
    "#cf-reset-enlist-tags",
  ) as HTMLButtonElement;
  const reset_load_tags_button = container.querySelector(
    "#cf-reset-load-tags",
  ) as HTMLButtonElement;
  ...
  model_option.addEventListener("change", () =>
    save_value("model", model_option.value),
  );
  context_input.addEventListener("input", () =>
    save_value("context", context_input.value),
  );
  initial_display.addEventListener("change", () =>
    save_value("initial_display", initial_display.checked),
  );
  // 2025.04.13: Add event listeners for tag inputs
  enlist_tags.addEventListener("input", () =>
    save_value("enlist_tags", enlist_tags.value),
  );
  load_tags.addEventListener("input", () =>
    save_value("load_tags", load_tags.value),
  );
  ...
  if (reset_enlist_tags_button) {
    reset_enlist_tags_button.addEventListener("click", () => {
      ...
      console.log(
        "[CleverFiller] Reset enlist tags to default:",
        DEFAULT_ENLIST_TAGS,
      );
    });
  }
  ...
  if (reset_load_tags_button) {
    reset_load_tags_button.addEventListener("click", () => {
      ...
      console.log(
        "[CleverFiller] Reset load tags to default:",
        DEFAULT_LOAD_TAGS,
      );
    });
  }
...
function reset_tags_to_default(): void {
  ...
  const enlistTagsInput = document.querySelector(
    "#cf-enlist-tags",
  ) as HTMLInputElement;
  const loadTagsInput = document.querySelector(
    "#cf-load-tags",
  ) as HTMLInputElement;
...
function createUI(): void {
  ...
  const cleverfiller_container = container.querySelector(
  ...
  const loadingText = cleverfiller_container.querySelector(
    "#cf-console-log",
  ) as HTMLElement;
  ...
  const hide_button = cleverfiller_container.querySelector(
    "#cf-hide-button",
  ) as HTMLButtonElement;
  const enlist_button = cleverfiller_container.querySelector(
    "#cf-enlist-button",
  ) as HTMLButtonElement;
  const run_button = cleverfiller_container.querySelector(
    "#cf-run-button",
  ) as HTMLButtonElement;
  const reset_button = cleverfiller_container.querySelector(
    "#cf-reset-button",
  ) as HTMLButtonElement;
...
function add_dropdown_button_listener(
  cleverfiller_container: HTMLDivElement,
): void {
  const dropdown_button =
    cleverfiller_container.querySelector("#cf-load-button");
  const dropdown_container = cleverfiller_container.querySelector(
    ".cf-dropdown-container",
  );
  const preload_button =
    cleverfiller_container.querySelector("#cf-preload-button");
  const afterload_button = cleverfiller_container.querySelector(
    "#cf-afterload-button",
  );
  ...
  document.addEventListener("click", (event) => {
    if (
      dropdown_container &&
      !dropdown_container.contains(event.target as Node)
    ) {
  ...
  preload_button?.addEventListener("click", () => {
    ...
    hover_overlay_handler_for_load_button(
      scan_form_elements(customSelector),
      "preload",
    );
    update_load_count();
  });
...
function add_hide_button_listener(
  hide_button: HTMLButtonElement,
...
function add_reset_button_listener(
...
function add_run_button_listener(
  ...
  run_button.addEventListener("click", async () => {
    ...
    hover_overlay_handler_for_load_button(
      scan_form_elements(loadSelector),
      "preload",
    );
    ...
    const existingOverlays = document.querySelectorAll(
      ".cleverfiller-hover-overlay-add, .cleverfiller-hover-overlay-remove, .cleverfiller-hover-overlay-preload",
    );
    existingOverlays.forEach((overlay) => overlay.remove());
