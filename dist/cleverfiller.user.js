...
async function get_response(options, on_start) {
...
async function call_deepseek_api(prompt) {
...
function create_prompt(context, formData) {
    ...
    const isSelect = formData["elementType"] === "select" ||
    ...
    for (const element of elements) {
        ...
        if (isAlreadyEnlisted) {
            ...
            overlay.addEventListener("click", (e) => {
                ...
                cacheArray, [
                    ...dataArray,
                    extract_data_for_enlist_storage(element),
                ], // Pass updated data
                addCallback, removeCallback, addColor, removeColor, addLabel, removeLabel);
            });
        }
...
function get_label_text(element) {
    ...
    if (parentRow) {
        ...
        if (cells.length > 0) {
            ...
            if (cells[0].matches('td[class*="label" i], th[class*="label" i]') &&
                cells[0].textContent) {
...
function add_dropdown_button_listener(cleverfiller_container) {
    ...
    document.addEventListener("click", (event) => {
        if (dropdown_container &&
            !dropdown_container.contains(event.target)) {
